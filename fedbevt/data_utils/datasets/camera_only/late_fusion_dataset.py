...
import fedbevt
from fedbevt.data_utils.datasets.camera_only import base_camera_dataset
from fedbevt.utils import common_utils


class CamLateFusionDataset(base_camera_dataset.BaseCameraDataset):
    def __init__(self, params, visualize, train=True, test=False, client=None):
    ...
    def get_item_test(self, base_data_dict):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            distance = common_utils.cav_distance_cal(selected_cav_base,
                                                     ego_lidar_pose)
            if distance > fedbevt.data_utils.datasets.COM_RANGE:
                continue
    ...
    def collate_batch(self, batch):
        ...
        for i in range(len(batch)):
            ...
            for agent_id, _ in cur_scene_data.items():
                ...
                mask_up, mask_left, mask_right, mask_down = self.amcm_process(dynamic_bev.shape)
                camera_ids =  self.params['clients'][self.client]['camera_idx']
                color_black = 0
                if 0 not in camera_ids:
                    dynamic_bev[mask_up] = color_black
                if 1 not in camera_ids:
                    dynamic_bev[mask_left] = color_black
                if 2 not in camera_ids:
                    dynamic_bev[mask_right] = color_black
                if 3 not in camera_ids:
                    dynamic_bev[mask_down] = color_black
    ...
    def amcm_process(self, img_shape):
        mask_up = np.zeros(img_shape, dtype=bool)
        mask_left = np.zeros(img_shape, dtype=bool)
        mask_right = np.zeros(img_shape, dtype=bool)
        mask_down = np.zeros(img_shape, dtype=bool)

        size = img_shape[0]  # assumes square image
        half_size = size // 2

        for y in range(size):
            for x in range(size):
                if x >= y and x < size - y:  # top-right quadrant
                    mask_up[y, x] = True
                elif x < y and x < size - y:  # top-left quadrant
                    mask_left[y, x] = True
                elif x < y and x >= size - y:  # bottom-left quadrant
                    mask_down[y, x] = True
                else:  # bottom-right quadrant
                    mask_right[y, x] = True

        mask_up[half_size:, :] = False
        mask_left[:, half_size:] = False
        mask_right[:, :half_size] = False
        mask_down[:half_size, :] = False

        return mask_up, mask_left, mask_right, mask_down
