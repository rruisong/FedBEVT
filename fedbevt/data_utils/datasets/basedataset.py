...
import fedbevt.utils.pcd_utils as pcd_utils
from fedbevt.utils.camera_utils import load_rgb_from_files
from fedbevt.data_utils.augmentor.data_augmentor import DataAugmentor
from fedbevt.config.yaml_utils import load_yaml
from fedbevt.utils.pcd_utils import downsample_lidar_minimum
from fedbevt.utils.transformation_utils import x1_to_x2
...
class BaseDataset(Dataset):
    ...
    def load_camera_files(self, cav_path, timestamp):
        ...
        if len(self.params["clients"][self.client]['camera_idx']) == 4:
            return [camera0_file, camera1_file, camera2_file]
        else:
            camera_list = [camera0_file, camera1_file, camera2_file, camera3_file]
            return [camera_list[i] for i in self.params["clients"][self.client]['camera_idx']]
