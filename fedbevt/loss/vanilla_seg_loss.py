...
class VanillaSegLoss(nn.Module):
    ...
    def logging(self, round, client_id, epoch, batch_id, batch_len, writer, pbar=None):
        """
        ...
        if pbar is None:
            print("[Round %d] || [Client %s] || [epoch %d][%d/%d] || Loss: %.4f || static Loss: %.4f"
                " || Dynamic Loss: %.4f" % (
                    round, client_id,
                    epoch, batch_id + 1, batch_len,
                    total_loss.item(), static_loss.item(), dynamic_loss.item()))
        else:
            pbar.set_description("[Round %d] || [Client %s] || [epoch %d][%d/%d], || Loss: %.4f || static Loss: %.4f"
                  " || Dynamic Loss: %.4f" % (
                      round, client_id,
                      epoch, batch_id + 1, batch_len,
                      total_loss.item(), static_loss.item(), dynamic_loss.item()))
